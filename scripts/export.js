...
const fetchResponse = () => {
  return new Promise((res, rej) => {
    try {
      const req = http.request('http://localhost:8181/#/', response => res(response.statusCode));
...
const convert = async() => {
  ...
  try {
    ...
    directories.forEach(async(dir) => {
      ...
      await page.goto('http://localhost:8181/#/resume/' + dir.name, {waitUntil: 'networkidle', networkIdleTimeout: 5E3});
